...
export function isValid(code) {
    ...
    if (sepIdx === -1)
        return false;
    if (clean.lastIndexOf(SEPARATOR) !== sepIdx)
        return false;
    if (sepIdx % 2 !== 0 || sepIdx > SEPARATOR_POSITION)
        return false;
    // Check padding
    ...
    if (padIdx > -1) {
        ...
        if (padIdx % 2 !== 0)
            return false;
        if (padIdx > SEPARATOR_POSITION)
            return false;
        const padding = clean.slice(padIdx, sepIdx);
        if (!/^[0]+$/.test(padding))
            return false;
        if (clean.length > sepIdx + 1)
            return false; // No chars after + if padded
    }
    // Check characters
    for (let i = 0; i < clean.length; i++) {
        const ch = clean[i];
        if (ch === SEPARATOR || ch === PADDING_CHARACTER)
            continue;
        if (CODE_ALPHABET.indexOf(ch) === -1)
            return false;
    }
    return true;
}
 ...
 */
export function encode(latitude, longitude, codeLength = 10) {
 ...
 */
export function shorten(code, referenceLat, referenceLng) {
    if (!isFull(code)) {
    ...
    const range = Math.max(Math.abs(referenceLat - codeArea.centerLatitude), Math.abs(referenceLng - codeArea.centerLongitude));
 ...
 */
export function recoverNearest(shortCode, referenceLat, referenceLng) {
    if (isFull(shortCode))
