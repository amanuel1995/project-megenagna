...
export interface RegionMetadata {
  code: string; // 2-letter ISO/National code (e.g., 'AA', 'OR', 'AM')
  nameEn: string;
  nameAm: string;
  nameOm?: string;
  nameTi?: string;
...
export interface SubDivisionMetadata {
  code: string; // 2 or 3 letter code (e.g. 'BO', 'LK', 'KK')
  nameEn: string;
  nameAm: string;
  nameOm?: string;
  centerLat: number;
...
export const ADDIS_ABABA_SUBCITIES: Record<string, SubDivisionMetadata> = {
...
export const ETHIOPIA_REGIONS: Record<string, RegionMetadata> = {
  AA: {
    code: 'AA',
    nameEn: 'Addis Ababa',
    nameAm: 'አዲስ አበባ',
