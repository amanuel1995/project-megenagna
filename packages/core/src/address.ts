...
import { encode, decode, shorten, recoverNearest, isFull, isShort, isValid, CodeArea } from './olc.js';
import { ETHIOPIA_REGIONS, ETHIOPIA_BOUNDS, RegionMetadata, SubDivisionMetadata } from './constants.js';
...
export interface MegenagnaAddress {
  ...
  woreda?: string; // e.g., "W03"
  houseNumber?: string;
  shortCode: string; // e.g. "8FW4+9X"
  fullCode: string; // e.g. "6GW78FW4+9X"
  formatted: string; // e.g. "ET-AA-LK-W03-8FW4+9X"
  latitude: number;
  longitude: number;
  bounds: CodeArea;
}
...
export interface EncodeOptions {
  ...
  houseNumber?: string;
  codeLength?: number; // default 10 (~14x14m)
}
 ...
 */
export function detectAdminContext(latitude: number, longitude: number): {
  region: RegionMetadata;
  subDivision?: SubDivisionMetadata;
} {
  let closestRegion: RegionMetadata = ETHIOPIA_REGIONS.AA;
  ...
  let closestSubDiv: SubDivisionMetadata | undefined = undefined;
...
export function fromCoordinates(
  ...
  if (subDiv) parts.push(subDiv.code);
  if (woredaStr) parts.push(woredaStr);
  if (options.houseNumber) parts.push(`H${options.houseNumber}`);
 ...
 */
export function parseAddress(input: string): MegenagnaAddress {
 ...
 */
export function toFaydaPayload(address: MegenagnaAddress): {
  country: 'ETH';
  region_code: string;
